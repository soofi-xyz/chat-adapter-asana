...
| --- | --- | --- |
| [`@soofi/chat-adapter-asana`](./packages/chat-adapter-asana) | Chat SDK `Adapter` for Asana. Tasks become threads, comments become messages, task completion is a first-class message type, and native Asana emoji reactions + file attachments are supported. | `@soofi/chat-adapter-asana` |
...
- A thread **starts** when a task is assigned to the bot. The first message in the thread is the task description (`raw.kind === "task_description"`).
- A thread **continues** when the bot is @-mentioned in a comment on the same task.
- Bot responses tag the person who sent the message by rendering an Asana inline `<a data-asana-gid="…">@Name</a>` link.
- **Emoji reactions** use Asana's native `reaction_summary` feature via `PUT /stories/{gid}` with a `reactions: [{ emoji, reacted }]` body (the replacement for the legacy `likes` system). Both `addReaction` and `removeReaction` call this endpoint — passing `reacted: true` adds the bot's reaction, `reacted: false` removes it. Chat SDK `EmojiValue` objects (e.g. `emoji.eyes`, `emoji.thumbs_up`) and shortcode strings are normalized to unicode via Chat SDK's `EmojiResolver` before the request. Reactions are scoped to the authenticated bot user.
- **File attachments** uploaded through `thread.post({ files: [...] })` are translated into Asana attachment uploads on the owning task.
- **Task completion** is emitted as a distinct message with `raw.kind === "task_completed"`. Use `isAsanaTaskCompletionMessage(message)` to detect it.
